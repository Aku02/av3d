...
def rotate_camera_along_y(extrinsic_matrix, angle_deg):
    angle_rad = np.radians(angle_deg)
    rotation_matrix = np.array([[1, 0, 0],
                                [0, np.cos(angle_rad), -np.sin(angle_rad)],
                                [0, np.sin(angle_rad), np.cos(angle_rad)]])
    rotated_extrinsic = np.dot(extrinsic_matrix[:3, :3], rotation_matrix)
    extrinsic_matrix[:3,:3] = rotated_extrinsic
    return extrinsic_matrix

def rotate_camera_along_z(extrinsic_matrix, angle_deg):
    angle_rad = np.radians(angle_deg)
...
class SubjectLoader(CachedIterDataset):
    ...
    def fetch_data(self, index):
        ...
        # ray_list = []
        # breakpoint()
        w2c = rotate_camera_along_y(w2c, 270)
        # w2c = rotate_camera_along_z(w2c, 180)
