...
from av3d.utils.structures import Bones, Cameras, namedtuple_map, Rays
...
class SubjectLoader(CachedIterDataset):
    ...
    SPLIT = ["all", "train", "val_ind", "val_ood", "val_view", "test", "mdm"]
    ...
    def preprocess(self, data):
        ...
        dd = hand[-2:,:3,3]
        head = hand[-9,:3,3]
        dd1 = sample_points_on_sphere(dd[0].cpu(), 0.08, 50)
        dd2 = sample_points_on_sphere(dd[1].cpu(), 0.08, 50)
        dd3 = sample_points_on_sphere(head.cpu(), 0.15, 50)
        dum = torch.cat([dd1, dd2, dd3], dim=0)
        ...
        mask_size = (int(1024 * self.resize_factor), int(1024 * self.resize_factor))
        ...
        if self.num_rays is not None:  # usually this is in the training phase
            ...
            if self.version == 3:
                ...
                output = []
                for d in [alpha, image, rays.origins, rays.directions, rays.viewdirs, rays.radii]:
                    patches = []
                ...
                (density, pixels, rays_origins, rays_directions, rays_viewdirs, rays_radii) = output
                ...
                if np.random.rand() < 1.0:
                    ...
                    valid = alpha[o:-o, o:-o] > 0
                    (xs, ys) = np.where(valid)
                    idx = np.random.choice(len(xs), size=2, replace=False)
                    x, y = xs[idx], ys[idx]
                else:
                    x = np.random.randint(0, image.shape[0] - patch[0], size=2)
                    y = np.random.randint(0, image.shape[1] - patch[1], size=2)
                output = []
                for d in [alpha, image, rays.origins, rays.directions, rays.viewdirs, rays.radii]:
                    patches = []
                    for xi, yi in zip(x, y):
                ...
                ray_indices = ray_indices_1 #torch.cat([ray_indices_1, ray_indices_hand])
    ...
    def fetch_data(self, index):
        ...
        # feat_rz = resample_dp(feat, dp_bbox, mat2K(torch.from_numpy(K.reshape((3,3)))), 1024) 
        # feat_rz = feat_rz.squeeze(0)
        # feat_rz = feat_rz.permute(2,1,0)
        # feat_rz = feat_rz.permute(1,0,2)
        
        ############################################
        # feat_rz = torch.from_numpy(
        #         cv2.resize(
        #             cv2.undistort(feat_rz.numpy(), K, D),
        #             (0, 0),
        #             fx=self.resize_factor,
        #             fy=self.resize_factor,
        #             interpolation=cv2.INTER_AREA,
        ...
        return {
            ...
            # "feat_1024": feat, #[h, w, 16]
            "hand_points": hand,
            "K": K,
            "extrin": w2c,
            # "bbox": dp_bbox#torch.from_numpy(bbox)
        }
