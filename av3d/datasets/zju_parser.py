...
import torch
import sys
sys.path.insert(0,'av3d/tools/')
from dp_stuff.util_flow import readPFM
...
class SubjectParser:
    ...
    def load_feat(self, frame_id, camera_id):
        path = "av3d/data/zju/CoreView_313/Densepose/Camera (%s)/feat-%05d.pfm"%(camera_id+1, frame_id)
        data, scale = readPFM(path)
        return data  # shape [HEIGHT, WIDTH, 3], value 0 ~ 255
    
    def load_bbox(self, frame_id, camera_id):
        path = "av3d/data/zju/CoreView_313/Densepose/Camera (%s)/bbox-%05d.txt"%(camera_id+1, frame_id)
    ...
    def _create_splits(self):
        from av3d.utils.clustering import train_val_test_split
