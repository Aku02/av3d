...
class DFaustDataset(Dataset):
    def __init__(self, dataset_path, stage, subject, points_per_frame=2000):
        ...
        split_path = hydra.utils.to_absolute_path('lib/dataset/snapshot_split.yml')
        with open(split_path, 'r') as stream:
            split = yaml.safe_load(stream)

        self.stage = stage
        # breakpoint()
        shape = np.load(os.path.join(dataset_path, 'shapes', '%s-shapes.npz'%subject))
        ...
        for name in split[str(subject)][stage]:
            # frame_name_act = glob.glob(os.path.join(dataset_path, 'points', 'male-3-casual','train'+ '_'  +'*.npz'))
            frame_name_act = glob.glob(os.path.join(dataset_path, 'points', str(subject), name  + '_'  +'*.npz'))
