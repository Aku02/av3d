...
def normalize(x, offset, scale, z_ratio):
    x_normalized = x.clone()
    x_normalized -= offset
    x_normalized /= scale
    x_normalized[..., -1] *= z_ratio
    return x_normalized

def denormalize(x, offset, scale, z_ratio):
    x_denormalized = x.clone()
    x_denormalized[..., -1] /= z_ratio
...
# def masked_softmax(vec, mask, warp ,dim=-1, mode='softmax', soft_blend=1):
#     if mode == 'softmax':
#         # breakpoint()
...
#         output = torch.distributions.Bernoulli(probs=output).logits
#         # breakpoint()
#         output, indices = torch.max(output, -2)#[0]
...
#         vec[~mask] = -15.94
#         # breakpoint()
#         output, indices = torch.max(vec, -2)#[0]
#         warp = torch.gather(
#             warp,
#             -2,
#             indices.unsqueeze(-2).expand(list(warp.shape[:-2]) + [1, 3]),
...
def masked_softmax(vec, mask, warp ,dim=-2, mode='softmax', soft_blend=1):
    if mode == 'min':
        vec[~mask] = -15.94
        # breakpoint()
        vec = torch.abs(vec)
        output, indices = torch.min(vec, -2)#[0]
        warp = torch.gather(
            warp,
            -2,
            indices.unsqueeze(-2).expand(list(warp.shape[:-2]) + [1, 3]),
        ...
        #     J_inv,
        #     -3,
        #     indices.unsqueeze(-1).unsqueeze(-1).expand(list(J_inv.shape[:-3]) + [1, 3, 3]),
        # ).squeeze(-3)
    return output, warp#, #J_inv
