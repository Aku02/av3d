...
class SNARFModel(pl.LightningModule):
    ...
    def forward(self, pts_d, smpl_tfs, smpl_params, eval_mode=True, calc_time=False):
        ...
        # (Pdb) smpl_tfs.shape
        # torch.Size([1, 24, 4, 4])
        # (Pdb) smpl_params.shape
        # torch.Size([1, 86])
        # (Pdb) smpl_params[:,7:-10].shape
        # torch.Size([1, 69])
        # (Pdb) intermediates['valid_ids'].shape --> masks
        # torch.Size([1, 35937, 9]) 
        # (Pdb) intermediates['J_inv'].shape
        # torch.Size([1, 35937, 9, 3, 3])
        # (Pdb) self.smpl_server.tfs_c_inv.shape
        # torch.Size([24, 4, 4])
        # (Pdb) pts_c.shape
        # torch.Size([1, 323433, 3])
        # (Pdb) pts_d_split.shape --> [B, num_point]
        # torch.Size([1, 35937, 3])
        # (Pdb) num_point 35937, num_init 9
        ...
        for pts_d_split in torch.split(pts_d, batch_points, dim=1):
            ...
            pts_c, intermediates = self.deformer(pts_d_split, None, smpl_tfs, eval_mode=eval_mode)
