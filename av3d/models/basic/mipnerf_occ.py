...
from av3d.models.basic import mip_utils
class MipNerfModel(nn.Module):
    ...
    def __init__(
        ...
        ray_shape: str = "cone",
        density_activation=torch.nn.Softplus(beta=1),  # Density activation.
        ...
        self.near_plane = None#torch.tensor(10, device="cuda:0")
        self.far_plane = None #torch.tensor(1000, device="cuda:0")
        ...
        resolution=[64, 64, 64],
        contraction_type=self.contraction_type).to("cuda:0")
    ...
    def forward(self, rays, color_bkgd, randomized: bool = True, **kwargs):
        ...
        def rgb_sigma_fn(t_starts, t_ends, ray_indices):
            ...
            t_dirs = rays_d[ray_indices]
            positions = t_origins + t_dirs * (t_starts + t_ends) / 2.
            density, feature = self.query_density(positions) 
            rgb = self._query_rgb(t_dirs, feature)
...
def volumetric_rendering(rgb, density, t_vals, dirs, color_bkgd, randomized):
    ...
    # density = torch.where(density > 0, 1.0, 0.0) # testing
