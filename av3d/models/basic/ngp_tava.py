...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        input_dim: int = 3, #60,  # The number of input tensor channels.
        ...
        self.mlp_base = tcnn.NetworkWithInputEncoding(
            ...
            n_output_dims=self.num_sigma_channels + self.net_width,
            encoding_config={
                "otype": "HashGrid",
                "n_levels": n_levels,
        ...
        #     },
        # )
        in_features = hidden_features = self.net_width
        ...
        if self.num_rgb_channels > 0:
            ...
            n_input_dims=in_features,
            n_output_dims=self.num_rgb_channels,
            network_config={
                "otype": "FullyFusedMLP",
                "activation": "ReLU",
    ...
    def _query_sigma(self, x, return_feat=True):
        ...
        if x.shape[0] == 0:
            if return_feat:
                return x.new_zeros(0, 1), x.new_zeros(0, self.net_width)
            else:
            ...
            #     return raw_sigma
            x = (
                self.mlp_base(inputs.view(-1, self.input_dim))
                .view(list(x.shape[:-1]) + [self.num_sigma_channels + self.net_width])
                .to(x)
            )
            density_before_activation, base_mlp_out = torch.split(
                x, [self.num_sigma_channels, self.net_width], dim=-1
            )
    ...
    def _query_rgb(self, raw_feat, condition=None):
        ...
        # breakpoint()
        # raw_rgb = self.color_net(x).float()
        return raw_rgb
