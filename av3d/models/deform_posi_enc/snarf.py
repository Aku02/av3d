...
from av3d.models.basic.mlp import StraightMLP
...
from av3d.utils.bone import closest_distance_to_points, sample_on_bones
from av3d.utils.knn import knn_gather
from av3d.utils.point import homo
...
class _SkinWeightsNet(nn.Module):
    def __init__(self, n_transforms: int):
        ...
        self.net = StraightMLP(
            net_depth=4,
            net_width=128,
            input_dim=self.posi_enc.out_dim,#3
...
class SNARFDPEncoder(nn.Module):
    ...
    def get_extra_losses(
        ...
        losses["loss_bone_occ"] = F.mse_loss(weights, weights_gt)
        ...
        if pre:
            ...
            ##################################
            # meshes = Meshes(verts=self.sa.verts.float().unsqueeze(0)*1, faces=self.sa.faces_idx.unsqueeze(0))
            # points = ops.sample_points_from_meshes(meshes, num_samples = 200000).squeeze(0)
            # points = points#.detach()
            # points += torch.randn_like(points) * 0.2 * 0.5
            # points_neighbs = points + torch.randn_like(points) * 0.01
            # points_neighbs = points + torch.randn_like(points) * 0.01
            outs = geometry(points.squeeze(0), output_normal = True)
            # outs_neigh = geometry(points_neighbs, output_normal = True)
            points_normal, points_sdf = outs['normal'], outs['sdf_grad']
            # points_neighbs_normal, points_neighbs_sdf = outs_neigh['normal'], outs_neigh['sdf_grad']
            # density_normal, _ = radiance_field.query_normal(points.squeeze(0), self.posi_enc)
            ...
            # loss_density = F.mse_loss(density_normal, normals)
            print("sdf loss", loss_normals)
            # print("density loss", loss_density)
            ...
            loss_normals = F.mse_loss(points_normal, self.normals)
            # outs = geometry(points, output_normal = True)
            # points_normal, points_sdf = outs['normal'], outs['sdf_grad']
            canonical_normals = torch.nn.functional.normalize(points_sdf, dim=1)
            # breakpoint()
            assert canonical_normals.shape[1] == 3
            ...
            # sdf_consistency = torch.mean(outs["sdf"] * outs["sdf"])
            #####################
            sdf_consistency = torch.mean(sdf_const * sdf_const)
