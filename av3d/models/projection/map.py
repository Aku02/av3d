...
class SurfaceAlignedConverter:
    ...
    def __init__(self):
        ...
        obj_path = '/home/karthi43/working/tava_working/tava/tava/models/projection/origin_can.obj'
    ...
    def load_mesh_topology(self, verts, faces_idx, cache_path='/home/karthi43/working/tava_working/tava/tool/zju_smpl/cache'):
    ...
    def _calculate_normals(self, points, nearest, triangles, normals_triangles):
        barycentric = points_to_barycentric(triangles, nearest)
        normal_at_s = barycentric_to_points(normals_triangles, barycentric)
        # contains = ((points - nearest) * normal_at_s).sum(1) < 0.0
        # contains = 1 - 2*contains
            ...
            for v in faces_idx[i]:
                ...
                while idx < verts_to_corres_faces.shape[1]:
                    if verts_to_corres_faces[v][idx] < 0:
                        ...
                        idx += 1
        verts_to_corres_faces = verts_to_corres_faces.clone()
        for i in range(len(faces_idx)):
            for v in faces_idx[i]:
