...
import torch
from av3d.utils.point import homo, transform_points
from av3d.utils.structures import Bones, Rays
...
def closest_distance_to_rays(bones: Bones, rays: Rays) -> Tuple[Tensor]:
    ...
    try:
        ...
        b = torch.zeros(b.shape[0], 1, 3, device=b.device)
        v = torch.zeros(v.shape[0], 1, 3, device=v.device)
        r = b - a
        # b = .zeros_like(b.shapr[0], 1, 3)
    ru = _dot(r, u)
    rv = _dot(r, v)
    uu = _dot(u, u)
    uv = _dot(u, v)
