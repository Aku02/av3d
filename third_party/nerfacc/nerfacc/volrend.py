...
def rendering(
    ...
    if rgb_sigma_fn is not None:
        if t_starts.shape[0] != 0:
            rgbs, sigmas, x_warp, mask, normal, sdf_grad, sdf = rgb_sigma_fn(t_starts, t_ends, ray_indices)
        else:
            rgbs = torch.empty((0, 3), device=t_starts.device)
            sigmas = torch.empty((0,), device=t_starts.device)
            x_warp = torch.empty((0, 3), device=t_starts.device)
            normal = torch.empty((0, 3), device=t_starts.device)
            sdf_grad = torch.empty((0, 3), device=t_starts.device)
            sdf = torch.empty((0, 1), device=t_starts.device)
    ...
    x_warp = accumulate_along_rays(
        weights, values=x_warp, ray_indices=ray_indices, n_rays=n_rays
    )
    
    normal = accumulate_along_rays(
        weights, values=normal, ray_indices=ray_indices, n_rays=n_rays
    )
    
    sdf_grad = accumulate_along_rays(
        weights, values=sdf_grad, ray_indices=ray_indices, n_rays=n_rays
    )
    
    # canos = accumulate_along_rays(
    #     weights, ray_indices, values=cano, n_rays=n_rays
    # )
    sdf = accumulate_along_rays(
        weights, values=sdf, ray_indices=ray_indices, n_rays=n_rays
    )
    ...
    return colors, opacities, depths, x_warp, normal, sdf_grad, sdf, extras
