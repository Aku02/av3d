...
class ControlNetGuidance(BaseObject):
    ...
    def compute_grad_du(
        ...
        **kwargs,
        ...
        if need_loss:
            ...
            gt_rgb = edit_images.requires_grad_(True)
            guidance_out.update(
                {
                    "loss_l1": F.mse_loss(
                        rgb_BCHW_512, gt_rgb.permute(0, 3, 1, 2)
                    ).sum(),
    ...
    def denoise_stuff(self, 
        ...
        clean = self.decode_latents(latents)
        clean = F.interpolate(clean, (H, W), mode="bilinear")

    def __call__(
        self,
        rgb: Float[Tensor, "B H W C"],
        cond_rgb: Float[Tensor, "B H W C"],
        ori_rgb: Float[Tensor, "B H W C"],
        ...
        **kwargs,
        ...
        if self.cfg.use_du:
            grad = self.compute_grad_du(
                latents, rgb_BCHW_512, image_cond, cond_rgb, ori_rgb, text_embeddings, **kwargs
            )
