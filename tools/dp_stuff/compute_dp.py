...
# try:
detbase='av3d/av3d/detectron2/'
# sys.path.insert(0,'%s/projects/DensePose/'%detbase)

from av3d.utils.cselib import create_cse, run_cse
# except:
#     detbase='/home/user/working/detectron2_old/'
#     sys.path.insert(0,'%s/projects/DensePose/'%detbase)
#     from utils.cselib import create_cse, run_cse

# sys.path.insert(0,'av3d/tools/dp_stuff/')
from av3d.utils.io import save_vid, visObj
...
cam_num = sys.argv[3]
cam_num = int(cam_num)
odir='av3d/data/zju/CoreView_%s'%(seqname)
imgdir= '%s/Camera (%d)/'%(odir, cam_num)
maskdir='%s/mask_cihp/Camera (%d)/'%(odir, cam_num)
dpdir='%s/Densepose/Camera (%d)'%(odir,cam_num)
...
counter=0 
# frames = []
cam = 'Camera (%d)'%(cam_num)
for i,path in enumerate(sorted(glob.glob('%s/*'%imgdir))):
    print(path)
    img = cv2.imread(path)
    msk = cv2.imread(path.replace('CoreView_313/'+ cam, 'CoreView_313/mask_cihp/'+ cam).replace('.jpg', '.png'),0)
    h,w = img.shape[:2]
